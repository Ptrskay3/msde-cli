...
fn parse_uuid<'a>(input: &mut &'a str) -> PResult<&'a str> {
    delimited(
        space0,
        preceded(
            literal("\""),
...
/// Parses two-element simple Elixir ok and error tuples that usually come from the Game.start and Game.sync calls.
pub fn parse_simple_tuple<'a>(input: &mut &'a str) -> PResult<ElixirTuple<'a>> {
    alt((parse_error, parse_ok)).parse_next(input)
}
